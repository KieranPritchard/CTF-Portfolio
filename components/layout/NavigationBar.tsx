/**
 * Fixed floating navigation bar with responsive desktop links and a mobile dropdown menu.
...
/** Primary site navigation links displayed in the nav bar. */
const navItems = [
    { label: "Home", href: "/" },
...
export function NavigationBar() {
    ...
    return (
        <div className="fixed top-4 sm:top-6 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] sm:w-auto">
            <nav className="gradient-border-frosted flex items-center justify-between sm:justify-center gap-1 px-3 py-1.5 sm:py-2 bg-background/80 backdrop-blur-md supports-[backdrop-filter]:bg-background/60 rounded-full shadow-sm relative">
                ...
                <AnimatePresence>
                    {isOpen && (
                        <motion.div 
                            ...
                            exit={{ opacity: 0, y: -10 }}
                            className="gradient-border-frosted absolute top-full left-0 right-0 mt-2 bg-background/95 backdrop-blur-md rounded-2xl shadow-xl overflow-hidden sm:hidden flex flex-col p-2"
                        >
                            {navItems.map((item) => (
                                <Link
                                    key={item.href}
